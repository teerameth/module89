...
/// Narwhal negamax function ///
void narwhal(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, Color us){
    if (depth == MAX_PLY) {
        return evaluate(pos);
    }
    else{
        best_value = -VALUE_INFINITE;
        for (const auto& m : MoveList<LEGAL>(pos)){                         // Loop by legal move generator
            pos.do_move(m, ss);                                             // do move & append to stack
            bestCandidate = -narwhal(pos, ss, -alpha, -beta, depth+1,~us);  // Apply nega max recursion
            best_value = max(best_value, bestCandidate);                    // replace bestValue with betterValue (may be?)
            pos.undo_move(m);                                               // undo move after recursion
...
void Thread::search() {
    ...
    for (const auto& m : MoveList<LEGAL>(pos)){
        pos.do_move(m, ss);
        value = narwhal(pos, ss, -VALUE_INFINITE, VALUE_INFINITE, 0, us);   // init alpha=-infinity, beta=infinity
        pos.undo_move(m);        
        if(best_value < value){                                             // Update with better candidate
            best_value = value;
            best_move = m;
        }
    }
    // Append result in main thread
    mainThread->iterValue[iterIdx] = bestValue;
    mainThread->move[iterIdx] = best_move;
