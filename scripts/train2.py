...
def loadjson(path, objectsofinterest, img):
    ...
    for i_line in range(len(data['objects'])):
        ...
        ## 3D bounding box (projected cuboid) ##
        points3d = []

        pointdata = info['projected_cuboid']
        # for i in [1, 2, 5, 6]:  # 4 point on upper surface (top-left, down-left, top-right, down-right)
        #     p = pointdata[i]
        #     points3d.append((p[0], p[1]))
        # Customized NDDS format (have only 4 points)
        for i in range(4):
            p = pointdata[i]
            points3d.append((p[0], p[1]))
